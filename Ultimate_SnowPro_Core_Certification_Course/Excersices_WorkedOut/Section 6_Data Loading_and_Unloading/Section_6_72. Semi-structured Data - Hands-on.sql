...
-- Create JSON file format object with STRIP_OUTER_ARRAY=TRUE

CREATE OR REPLACE FILE FORMAT JSON_FILE_FORMAT_1
TYPE='JSON',
FILE_EXTENSION=NULL,
DATE_FORMAT='AUTO',
TIME_FORMAT='AUTO',
...
-- select $1,$2,$3 from @film_stage/films.json;

COPY INTO FILMS_ELT
FROM @FILM_STAGE/films.json
FILE_FORMAT = JSON_FILE_FORMAT; -- force = true => reload the contents from stage

COPY INTO FILMS_ELT
FROM @FILM_STAGE/films.json
FILE_FORMAT = JSON_FILE_FORMAT_1 force=TRUE;

SELECT JSON_VARIANT FROM FILMS_ELT; -- Max limit 16mb per(single) row and column
...
SELECT 
json_variant:id as id, 
json_variant:Title as title, /* Returns None -> Because Title is case sensitive in JSON*/
...
select json_variant:title, json_variant:ratings from films_ELT;

select json_variant:title, json_variant:ratings from films_ELT where json_variant:title is not null and json_variant:ratings is not null;
...
SELECT * FROM TABLE(FLATTEN(INPUT => select json_variant:ratings from films_ELT where json_variant:title is not null and json_variant:ratings is not null limit 1)) ;

SELECT VALUE FROM TABLE(FLATTEN(INPUT => select json_variant:ratings from films_ELT where json_variant:title is not null and json_variant:ratings is not null limit 1));
...
$1:ratings
FROM @FILM_STAGE/films.json)
FILE_FORMAT = JSON_FILE_FORMAT_1
FORCE=TRUE;
...
/* TABLE FOR Match By Column Name (MBCN) */

CREATE OR REPLACE TABLE FILMS_ETL_MBCN (
ID STRING, 
TITLE STRING, 
RELEASE_DATE DATE, 
STARS ARRAY, 
...
COPY INTO FILMS_ETL_MBCN
FROM @FILM_STAGE/films.json
FILE_FORMAT = JSON_FILE_FORMAT_1
MATCH_BY_COLUMN_NAME = CASE_INSENSITIVE
FORCE=TRUE;

SELECT * FROM FILMS_ETL_MBCN;
